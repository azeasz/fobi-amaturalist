...
function ImageComments({ images = [], onRatingUpdate, source }) {
    ...
    useEffect(() => {
        if (source && images.length > 0 && onRatingUpdate) {
            fetchAllImageData();
        }
    }, [onRatingUpdate]);
    ...
    const fetchAllImageData = async () => {
        ...
        try {
            ...
            setFetchErrors(prev => ({
                ...prev,
                general: 'Gagal memuat data gambar'
            }));
        } finally {
            setIsLoading(false);
        }
    ...
    const fetchImageData = async (imageId) => {
        try {
            ...
            if (commentsData.success && ratingsData.success) {
                ...
                const mediaSource = ratingsData.data?.media_source || 'fobi';
            ...
            // Jika gagal mengambil data, tetap kembalikan objek dengan data minimal
            const imageInfo = images.find(img => img.id === imageId);
            return {
                imageId,
                imageUrl: imageInfo?.url || '',
            ...
            setFetchErrors(prev => ({
                ...prev,
                [imageId]: `Gagal memuat data untuk gambar #${imageId}`
            }));
            // Kembalikan null agar gambar ini tidak ditampilkan
            return null;
        }
    };
    ...
    const handleSubmitComment = async (e, imageId) => {
        ...
        try {
            ...
            const imageInfo = imageData.find(img => img.id === imageId);
            
            // Gunakan source parameter yang sesuai saat mengirim komentar
            ...
            <div className="bg-[#1e1e1e] rounded-lg p-6 border border-[#444]">
                ...
                <div className="text-center py-6 text-[#b0b0b0]">
                    {Object.keys(fetchErrors).length > 0 
                        ? 'Terjadi kesalahan saat memuat data media'
